...
For Japanese output, also read [Japanese editing patterns](references/japanese.md). The numbered rules below retain their stable IDs. Apply their language-independent intent, but English grammar and typography rules (articles, `-ing`, title case, English quotes and punctuation) apply only to English prose. Japanese 「」, parentheses, 、 and 。 are allowed when they clarify meaning. Never rewrite literal quotations, code, logs, or identifiers to satisfy a style rule. Confidence-bearing qualifications are findings, not filler.
