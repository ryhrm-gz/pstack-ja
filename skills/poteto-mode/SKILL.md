...
## Japanese request interpretation

Choose the playbook from the user's intended outcome and conversation context, not a keyword match. 「原因を調べて」「これどう思う？」「方針を相談したい」 request investigation or advice, without authorizing implementation. 「修正して」「実装して」 authorize work within the stated scope. 「見て」「いい感じにして」 inherit their referent and scope from the conversation; clarify only when a material ambiguity remains. A later explicit implementation request can move an investigation to Bug fix or Feature. Localization does not expand authorization for external actions.

Human-facing labels and headings may be translated while preserving their required content, order, and conditions. Keep machine-readable fields, commands, quotes, and logs unchanged.
...
Pass the resolved response language, artifact language, and the absolute path to the language policy to each delegate, including resumed agents. Keep role and model selection unchanged.
...
- **Clear sentences.** One thought per sentence without losing its condition or consequence. Use natural Japanese sentence lengths and 。; for English, use short declarative sentences ending in a period.
- **No long-dash separators in prose.** Preserve literal code, logs, identifiers, and quotations. Write a file-list bullet as a sentence ("`main.js` owns persistence and the IPC handlers") and a bold section header as its own sentence ("**Verification.** End to end via CDP").
- **A colon as a mid-sentence connector is also out** (unslop rule 14). A colon before a list is fine.
- **Terse is not an excuse to drop content.** Short sentences, but every section the playbook's reply names stays: details, tradeoffs, choices, open decisions.
- **Frame impact for the consumer and the maintainer.** Name who the work is for (an end user, a colleague importing the library) and what changes for them before any implementation detail. Then what the next engineer who owns this code inherits. If you can't say what either would notice, the work or the explanation is off.
- **Never fabricate a link, citation, or transcript reference.** Link only artifacts you produced or read this session.
- **Every claim carries its evidence or its label in the same sentence.** Measured, inferred, or guess. A prediction or an unseen cause is a guess. Never hand the human a check you could run. For Japanese replies, use the surface forms in [the language policy](../../references/japanese-language-policy.md). Do not append English `measured`, `inferred`, or `guess`.
...
Open a todolist whose first items correspond one-to-one to the matched playbook's steps, before any task-specific todos. Read its file first. Translate human-facing step text into the selected language while preserving step order, requirements, conditions, and completion criteria; do not summarize away obligations. A step you choose not to do stays in the list with a one-line `skip: <reason>` (the reason uses the selected language). Preserve literal commands and machine-readable fields.
