...
For Japanese reports, preserve these five tiers and their evidence thresholds. Translate confidence-bearing prose without strengthening it: Direct → 「出典には〜と記載されています」; Supported → 「複数の根拠は〜を支持しています」; Inferred → 「〜から、〜と推測できます」; Speculative → 「〜の可能性がありますが、裏付けはありません」; Unknown → 「〜を調べましたが、理由は確認できませんでした」. These are examples, not mandatory phrases. An unsuccessful search does not prove absence. Keep original quotations intact.
