...
When building every explorer, investigator, explainer, or synthesizer prompt, pass the resolved output language and the absolute path to the language policy. Translate human-facing headings, not evidence or identifiers. Preserve the investigation method and all confidence distinctions.
...
Take the synthesizer's output and present it to the user. You may lightly edit for clarity or add context from the conversation, but **preserve the confidence level when translating its wording; do not strengthen or weaken the claim**.
